...
public class Administrators extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbStamp = findViewById(R.id.stamp);
        dbStamp.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus)
                dbStamp.setHint("");
            else
                dbStamp.setHint("Марка");
        });
        dbModel = findViewById(R.id.model);
        dbModel.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus)
                dbModel.setHint("");
            else
                dbModel.setHint("Модель");
        });
        dbYear = findViewById(R.id.year);
        dbYear.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus)
                dbYear.setHint("");
            else
                dbYear.setHint("Год выпуска");
        });
        dbMileage = findViewById(R.id.mileage);
        dbMileage.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus)
                dbMileage.setHint("");
            else
                dbMileage.setHint("Пробег");
        });
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btnAdd:
                ...
                database.insert(DBHelper.TABLE_AUTO, null, contentValues);
                ...
                dbModel.clearFocus();
                dbYear.clearFocus();
                dbMileage.clearFocus();
                break;

            case R.id.btnClear:
                database.delete(DBHelper.TABLE_AUTO, null, null);
                TableLayout dbOutput = findViewById(R.id.dbOutput);
                dbOutput.removeAllViews();
                UpdateTable();
            ...
            default:
                ...
                database.delete(DBHelper.TABLE_AUTO, DBHelper.KEY_ID + " = ?", new String[]{String.valueOf((v.getId()))});

                contentValues = new ContentValues();
                Cursor cursorUpdater = database.query(DBHelper.TABLE_AUTO, null, null, null, null, null, null);
                if (cursorUpdater.moveToFirst()) {
                    ...
                    do {
                        if(cursorUpdater.getInt(idIndex) > realID) {
                            ...
                            database.replace(DBHelper.TABLE_AUTO, null, contentValues);
                        }
                        realID++;
                    }while(cursorUpdater.moveToNext());
                    if(cursorUpdater.moveToLast()) {
                        database.delete(DBHelper.TABLE_AUTO, DBHelper.KEY_ID + " = ?", new String[]{cursorUpdater.getString(idIndex)});
                    }
                    UpdateTable();
                }
                break;
